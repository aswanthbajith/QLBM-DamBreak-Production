"""
Phase F36: Quantum Processing Unit (QPU) Executor & Double Opt-In Safety Gateway.

Enforces:
- Strict double opt-in safety verification
- Dry-run transpilation and gate-depth reporting
- Automatic disk serialization to results/f36/
"""
...
from quantum.f36_observables_reconstruction import F36ObservablesReconstructor


class F36QPUExecutor:
    """
    Executes or dry-runs two-phase dam-break quantum circuits on physical hardware.
    """

    def __init__(self, nx: int = 2, ny: int = 2, bits_per_node: int = 4, results_dir: str = "results/f36"):
        self.nx = nx
        self.ny = ny
    ...
    def execute_live_qpu(self, shots: int = 4096) -> Dict[str, Any]:
        ...
        try:
            ...
            fields = F36ObservablesReconstructor.reconstruct_from_counts(
