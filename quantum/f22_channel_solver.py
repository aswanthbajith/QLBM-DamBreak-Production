"""
Phase F22: Multi-Timestep CPTP Quantum Channel Two-Phase Dam-Break Solver.

Executes:
rho_0 -> [ E_CSF -> E_BGK(F_ext) -> U_stream -> U_boundary ]^T -> Final Measurement Readout

Guarantees:
- Exact zeroth-moment mass conservation: sum f_out == rho_in (Zero mass leakage).
- Direct active coupling of CSF surface tension to fluid momentum.
- Autonomous execution across T timesteps without intermediate classical extraction.
"""
...
class PhaseF22CPTPChannelSolver:
    """
    Phase F22 CPTP Quantum Channel Two-Phase Solver with exact mass conservation.
    """
    ...
    def __init__(
        ...
        self.bgk_engine = F22ExactMassConservingBGKEngine(
            omega_f=omega_f0, omega_g=omega_g0, g_acc=g_acc, frac_bits=frac_bits
        )
    ...
    def _init_state(self):
        ...
        self.initial_total_mass_int = int(np.sum(self.f_reg))
        self.num_state_preparations += 1

    def step(self) -> Dict[str, Any]:
        """Executes one autonomous CPTP timestep E_step = U_bound o U_stream o E_BGK+CSF."""
        ...
        # 3. Exact Mass-Conserving Collision with CSF forcing
        ...
        for y in range(ny):
            for x in range(nx):
                ...
                # Add CSF force directly to momentum in conservative BGK
                f_out, g_out, meta = self.bgk_engine.evaluate_conservative_bgk_map(
                    f_in, g_in, F_ext=f_ext_node
                )
        ...
        # 4. Exact Spatial Streaming Permutation U_stream
        ...
        # 5. Exact Boundary Bounce-Back Involution U_boundary
        ...
        current_mass_int = int(np.sum(self.f_reg))
        mass_drift = abs(current_mass_int - self.initial_total_mass_int)

        return {
            "timesteps_completed": self.num_quantum_timesteps,
            "csf_is_uncomputed": csf_meta["is_uncomputed"],
            "is_autonomous": True,
            "mass_drift_int": mass_drift,
            "is_mass_conserved": (mass_drift == 0),
        }
