...
def apply_quantum_boundary(f_stream, g_stream):
    ...
    """
    f_out = np.array(f_stream, copy=True)
    g_out = np.array(g_stream, copy=True)
...
def build_two_phase_boundary_circuit(layout):
    ...
    if total_qubits <= 9:
        unitary_gate = UnitaryGate(B, label="BounceBack_B")
        qc.append(unitary_gate, range(total_qubits))
    else:
        from qiskit.circuit import Gate
        qc.append(Gate("BounceBack_B", total_qubits, []), range(total_qubits))
