"""
Phase F35: Quantum Processing Unit (QPU) Executor & Safety Gateway.

Enforces:
- Strict double opt-in safety verification
- Dry-run transpilation and gate-depth reporting
- Automatic disk serialization to results/f35/
"""
...
from quantum.f35_backend_discovery import F35BackendDiscovery
from quantum.f35_observables_reconstruction import F35ObservablesReconstructor


class F35QPUExecutor:
    """
    Executes or dry-runs two-phase dam-break quantum circuits on physical hardware.
    """

    def __init__(self, nx: int = 2, ny: int = 2, bits_per_node: int = 4, results_dir: str = "results/f35"):
        self.nx = nx
        self.ny = ny
    ...
    def execute_live_qpu(self, shots: int = 4096) -> Dict[str, Any]:
        ...
        try:
            ...
            fields = F35ObservablesReconstructor.reconstruct_from_counts(
