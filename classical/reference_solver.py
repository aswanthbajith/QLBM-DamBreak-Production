"""
CANONICAL CLASSICAL REFERENCE SOLVER
Reduced Two-Phase Lattice Boltzmann Dam-Break Hydrodynamics.

FROZEN GROUND TRUTH - DO NOT MODIFY AFTER CREATION.
This file serves as the absolute ground truth reference against which
all quantum and Carleman formulations are validated.
"""
import numpy as np
...
def run_two_phase_dambreak(nx=4, ny=4, timesteps=5, tau_f=0.8, tau_g=0.7, g_acc=-0.001):
    ...
    for t in range(1, timesteps + 1):
        f_curr, g_curr, phi_curr, rho_curr, u_curr = step_two_phase(f_curr, g_curr, tau_f, tau_g, g_acc)
        history.append({
            "step": t,
            "f": np.copy(f_curr),
            "g": np.copy(g_curr),
            "phi": np.copy(phi_curr),
            "rho": np.copy(rho_curr),
            "u": np.copy(u_curr),
            "total_liquid_mass": float(np.sum(phi_curr)),
